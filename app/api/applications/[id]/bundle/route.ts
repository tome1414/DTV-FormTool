...
// type=admin  → 受入れレター + インボイス + 既存PDF一式
// type=bank   → 残高証明書 + 取引履歴（全ページ）
const ADMIN_BUNDLE_KEYS = ["acceptanceLetter", "invoice", "existingPdfBundle"] as const;
const BANK_BUNDLE_KEYS = ["bankStatement", "bankStatementHistory"] as const;
...
// GET /api/applications/[id]/bundle?type=admin  (default)
// GET /api/applications/[id]/bundle?type=bank
export async function GET(request: NextRequest, { params }: Params) {
  const sessionUser = await getSessionUser();
  if (!sessionUser) return unauthorized();
  if (!await canAccessApplication(sessionUser, params.id)) return forbidden();

  const bundleType = request.nextUrl.searchParams.get("type") ?? "admin";
  const isBank = bundleType === "bank";

  const requiredKeys = isBank ? [...BANK_BUNDLE_KEYS] : [...ADMIN_BUNDLE_KEYS];
  ...
  if (isBank) {
    ...
    for (const key of ADMIN_BUNDLE_KEYS) {
  ...
  const prefix = isBank ? "bank_bundle" : "bundle";
  const filename = `${prefix}_${app?.application_number ?? params.id}.pdf`;
